...
client
	...
	proc/getCor(amount)
		if(!amount) return
		src.mob.Cor += amount
		outputMessage(src,"<span style='color:yellow;font-size:2;font-weight:bold'>You were awarded </span><span style='color:white;font-size:2;font-weight:bold'>[amount]</span><span style='color:yellow;font-size:2;font-weight:bold'> Cor!</span>", "gametext")
	...
	proc/questRewards(Quest/quest)
		...
		if(quest.rewards["fragments"])
			getCor(quest.rewards["fragments"])
